          ...
          if (!err && row && row.count === 0) {
            ...
            db.serialize(() => {
              ...
              defaultItems.forEach(item => {
                stmt.run(item.id, item.itemName, item.category, item.price, item.quantity, item.img, new Date().toISOString(), (err) => {
                  if (err) console.error(`Failed to seed item ${item.itemName}:`, err.message);
                });
              });
              stmt.finalize((err) => {
                if (err) console.error("Error finalizing seeding statement:", err.message);
                else console.log("SQLite inventory database seeded successfully.");
              });
            });
          }
        });
...
// Seeding endpoint (manual trigger)
app.get('/api/seed', (req, res) => {
  ...
  db.serialize(() => {
    db.run("DELETE FROM inventory", [], (err) => {
      ...
      stmt.finalize((err) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json({ message: "Seeded database successfully with default items" });
      });
    });
  });
});
